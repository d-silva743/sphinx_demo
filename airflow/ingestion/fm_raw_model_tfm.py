...
from snowflake.snowpark.functions import concat, lit, col, when, lead, lag, to_timestamp, trim, row_number
...
def fetchWagerPayouts(session, customerName,batchId):
    pgsDf = session.table("primarygamestarted_raw").filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)
    pgeDf = session.table("gameended_raw").filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)
    
    pgsDf_cols = ['egmId','deviceId','denomId','eventDatetime','gameplaylogsequence','g2s_playercashableamt','customer','g2s_wageredcashableamt']
    pgeDf_cols = ['egmId','deviceId','denomId','eventDatetime','gameplaylogsequence','g2s_playercashableamt','customer']
    
    pgsDf = pgsDf.select(pgsDf_cols)
    pgstimeDf = pgsDf.withColumn('rectype',lit('w'))
        
    pgeDf = pgeDf.select(pgeDf_cols)
    pgetimeDf = pgeDf.withColumn('g2s_wageredcashableamt',lit(None))\
                .withColumn('rectype',lit('o'))

    wagerDf = pgstimeDf.unionAll(pgetimeDf)
    sortwagerDf = wagerDf.withColumn('sortpriority', F.when((wagerDf.rectype == 'w'), 1)\
                               .when((wagerDf.rectype == 'o'), 2))

    cashAmtDf = sortwagerDf.withColumn('next_G2S_PLAYERCASHABLEAMT',
                              F.lead(sortwagerDf.G2S_PLAYERCASHABLEAMT).over(
                            Window.partitionBy(sortwagerDf.EGMID,sortwagerDf.GAMEPLAYLOGSEQUENCE,sortwagerDf.DEVICEID,
                                               ).orderBy(sortwagerDf.EVENTDATETIME,sortwagerDf.sortpriority)))

    filterDf = cashAmtDf.withColumn('payout',cashAmtDf.next_G2S_PLAYERCASHABLEAMT - cashAmtDf.G2S_PLAYERCASHABLEAMT)

    filteredDf = filterDf.select(
                            filterDf.rectype,
                            filterDf.egmId, F.lit(None).alias('themeId'), F.lit(None).alias('themeName'), filterDf.deviceId,
    ...
    wagerPayoutsDf = filteredDf.withColumn('tranAmt', filteredDf.creditMeter - 
    ...
    primaryGamesDf = wagerPayoutsDf.select(
                                            ...
                                            wagerPayoutsDf.egmId, wagerPayoutsDf.themeId, F.lit(None).alias('themeName'), wagerPayoutsDf.deviceId,
                                            wagerPayoutsDf.denomId, wagerPayoutsDf.tranTimestamp, wagerPayoutsDf.gameplaylogsequence,
                                            wagerPayoutsDf.creditMeter, wagerPayoutsDf.tranAmt, wagerPayoutsDf.payout,
                                            wagerPayoutsDf.playerId, wagerPayoutsDf.customer,F.lit(None).alias('paytable'),wagerPayoutsDf.areaid,wagerPayoutsDf.location,wagerPayoutsDf.currencyid
                                           )
...
def fetchSecondaryGames(session, customerName,batchId):
    sgsDf = session.table('secondarygamestarted_raw').filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)
    sgeDf = session.table('secondarygameended_raw').filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)
    ...
    sgsDistinctDf = sgsDf.select([
                                    'id', 'egmId', 'eventDatetime', 'deviceId', 'denomId', 'gamePlaylogsequence', 
                                     'gcmdatetime', 'commserverid', 'customer',
                                     'g2s_secwageredamt'
                                ]) \
                        .distinct()
    ...
    sgeDistinctDf = sgeDf.select([
                                    'id', 'egmId', 'eventDatetime', 'deviceId', 'denomId', 'gameplaylogsequence', 
                                    'gcmdatetime','commserverid','customer',
                                    'g2s_secwonamt', 'g2s_secwoncnt','g2s_seclostcnt', 'g2s_sectiedcnt'
                                ]) \
                        .distinct()
    ...
    sgsDistinctDf = sgsDistinctDf.withColumn('rectype',lit('x'))\
    ...
    sgeDistinctDf = sgeDistinctDf.withColumn('rectype',lit('y'))\
                                    .withColumn('g2s_secwageredamt',lit(None))
    
    # select to columns in the same order to perform union on sgsDistinctDf and sgeDistinctDf dataframes
    ordered_cols = ['id', 'egmId', 'eventDatetime', 'deviceId', 'denomId', 'gameplaylogsequence','gcmdatetime','commserverid',
                    'customer','g2s_secwonamt', 'g2s_secwoncnt','g2s_seclostcnt', 'g2s_sectiedcnt','g2s_secwageredamt',
                    'rectype']
    ...
    sortwagerDf = bonusDf.withColumn('sortpriority', F.when((bonusDf.rectype == 'x'), 1)\
                               .when((bonusDf.rectype == 'y'), 2))
    ...
    filteredDf = sortwagerDf.select(
                            sortwagerDf.egmId, F.lit(None).alias('themeId'), F.lit(None).alias('themeName'), sortwagerDf.deviceId,
                            sortwagerDf.denomId,
                            ...
                            (
                                sortwagerDf.G2S_secwageredAmt -
                                                    F.lag(sortwagerDf.G2S_secwageredAmt,2).over(
                                                                                Window.partitionBy(sortwagerDf.egmId)
                                                                                        .orderBy(sortwagerDf.eventDatetime,sortwagerDf.sortpriority)
                                                                            )
                            ).alias('tranAmt'),
                            (
                            ...
                            F.lit(None).alias('playerId'),
                            sortwagerDf.customer,F.lit(None).alias('paytable'),sortwagerDf.rectype,
                            F.lit(None).alias('areaid'),
                            F.lit(None).alias('location'),
                            F.lit(None).alias('currencyid')                        
    )
    ...
    secondaryGamesDf = bonusPayoutsDf.select(
                                                ...
                                                bonusPayoutsDf.egmId, bonusPayoutsDf.themeId, F.lit(None).alias('themeName'), bonusPayoutsDf.deviceId,
                                                bonusPayoutsDf.denomId, bonusPayoutsDf.tranTimestamp, bonusPayoutsDf.gameplaylogsequence,
                                                bonusPayoutsDf.creditMeter, bonusPayoutsDf.tranAmt, bonusPayoutsDf.payout,
                                                bonusPayoutsDf.playerId, bonusPayoutsDf.customer,F.lit(None).alias('paytable'),bonusPayoutsDf.areaid,bonusPayoutsDf.location,bonusPayoutsDf.currencyid,
                                            )
...
def fetchBonusPayouts(session, customerName,batchId):
    bapdf = session.table("bonusawardpaid_raw").filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)

    BonusPayouts = bapdf.select (
                                    F.lit('bonus_award_paid').alias('rectype'), bapdf.egmId, 
...
def fetchCashEvents(session, customerName,batchId):
    nsdf = session.table("notestacked_raw").filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)
    vidf = session.table("voucherissued_raw").filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)
    vrdf = session.table("voucherredeemed_raw").filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)

    notestackeddf = nsdf.select(
                                    F.lit('n').alias('rectype'), nsdf.egmId, 
                                    F.lit(None).alias('themeId'),
                                    F.lit(None).alias('themeName'),
                                    F.lit(None).alias('deviceId'),
                                    F.lit(None).alias('denomId'),
    ...
    voucherissueddf = vidf.select(
                                    ...
                                    F.lit(None).alias('themeName'),
                                    F.lit(None).alias('deviceId'),
                                    F.lit(None).alias('denomId'), vidf.eventDatetime.alias('tranTimestamp'),
...
def fetchMiscEvents(session, customerName,batchId):
    ...
    gcadf = session.table("gamecomboactivated_raw").filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)
    ...
    pssdf = session.table("stm_raw").filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)
    psedf = session.table("stm_raw").filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)
    gpprdf = session.table("gameplayprofilereceived_raw").filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)
    gpsrdf = session.table("gameplaystatusreceived_raw").filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)
    crdf   = session.table("cabinetprofilereceived_raw").filter(F.col('customer') == customerName).filter(F.col('batch_id') == batchId)


    gamecomboactivateddf  = gcadf.select (
                                                F.lit('game_changed').alias('rectype'), gcadf.egmid,
    ...
    playersessionstarteddf = pssdf.select ( 
                                                F.lit('pss').alias('rectype'), pssdf.egmid,
    ...
    playersessionendeddf = psedf.select ( 
                                                F.lit('pse').alias('rectype'), psedf.egmid,
    ...
    gameplayprofilereceiveddf = gpprdf.select(
                                                F.lit('gppr').alias('rectype'), gpprdf.egmid,
                                                F.lit(None).alias('themeId'),
                                                F.lit(None).alias('themeName'),
    ...
    gameplaystatusreceiveddf = gpsrdf.select( 
                                                F.lit('gpsr').alias('rectype'), gpsrdf.egmid,
    ...
    cabinetprofilereceiveddf = crdf.select(
                                                F.lit('cbpr').alias('rectype'),crdf.egmid,
                                                F.lit(None).alias('themeId'),
                                                F.lit(None).alias('themeName'),
                                                F.lit(None).alias('deviceId'),
                                                F.lit(None).alias('denomId'),
    ...
    miscDf = gamecomboactivateddf.unionAll(playersessionstarteddf)\
...
def unionAllRectypes(primaryGamesDf, secondaryGamesDf, bonusPayoutsDf, cashDf, miscDf):
    ...
    rawDf = primaryGamesDf.unionAll(secondaryGamesDf).unionAll(bonusPayoutsDf).unionAll(cashDf).unionAll(miscDf)
...
def execute(customerName, batchId):
    ...
    rawDf = unionAllRectypes(primaryGamesDf, secondaryGamesDf,bonuspayouts, cashEvents, miscEvents)
